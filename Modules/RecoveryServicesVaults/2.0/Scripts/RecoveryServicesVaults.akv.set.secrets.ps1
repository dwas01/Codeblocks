<#
	.NOTES
		...
		File:		RecoveryServicesVaults.akv.set.secrets.ps1

		Purpose:	Set Recovery Services Vaults Key Secrets
	...
	.SYNOPSIS
		Set Recovery Services Vaults Key Secrets.
	...
	.DESCRIPTION
		...
		2) Set Recovery Services Vaults Parameters
	...
	.EXAMPLE
		Default:
		C:\PS>.\RecoveryServicesVaults.akv.set.secrets.ps1
...
if (-not [string]::IsNullOrWhiteSpace($PSBoundParameters['keyVaultName']))
{
	...
	#region - Recovery Services Vaults Parameters
	if (-not [string]::IsNullOrWhiteSpace($PSBoundParameters['recoveryServicesVaultsName']))
	{
		...
		$kvSecretParameters.Add("RecoveryServicesVaults--Name", $($recoveryServicesVaultsName))
	}
	else
	{
	...
	if (-not [string]::IsNullOrWhiteSpace($PSBoundParameters['recoveryServicesVaultsResourceId']))
	{
		...
		$kvSecretParameters.Add("RecoveryServicesVaults--ResourceId", $($recoveryServicesVaultsResourceId))
	}
	else
	{
	...
	{
		Write-Output "Recovery Services Vaults ResourceGroup: $recoveryServicesVaultsResourceGroup"
		$kvSecretParameters.Add("RecoveryServicesVaults--ResourceGroup", $($recoveryServicesVaultsResourceGroup))
	}
	else
	{
		Write-Output "Recovery Services Vaults ResourceGroup: []"
	}
	#endregion
