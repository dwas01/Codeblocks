<#
	.NOTES
		...
		File:		ApplicationSecurityGroups.akv.set.secrets.ps1

		Purpose:	Set Application Security Groups Key Secrets
	...
	.SYNOPSIS
		Set Application Security Groups Key Secrets.
	...
	.DESCRIPTION
		...
		2) Set Application Security Groups Parameters
	...
	.EXAMPLE
		Default:
		C:\PS>.\ApplicationSecurityGroups.akv.set.secrets.ps1
...
if (-not [string]::IsNullOrWhiteSpace($PSBoundParameters['keyVaultName']))
{
	...
	if (-not [string]::IsNullOrWhiteSpace($PSBoundParameters['applicationSecurityGroupName']))
	{
		...
		$kvSecretParameters.Add("ApplicationSecurityGroup--Name", $($applicationSecurityGroupName))
	}
	else
	{
	...
	if (-not [string]::IsNullOrWhiteSpace($PSBoundParameters['applicationSecurityGroupResourceId']))
	{
		...
		$kvSecretParameters.Add("ApplicationSecurityGroup--ResourceId", $($applicationSecurityGroupResourceId))
	}
	else
	{
	...
	if (-not [string]::IsNullOrWhiteSpace($PSBoundParameters['applicationSecurityGroupResourceGroup']))
	{
		...
		$kvSecretParameters.Add("ApplicationSecurityGroup--ResourceGroup", $($applicationSecurityGroupResourceGroup))
	}
	else
	{
