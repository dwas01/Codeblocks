<#
	.NOTES
		...
		File:		NotificationHubs.output.tests.ps1

		Purpose:	Test - Notification Hubs ARM Template Output Variables
	...
	.SYNOPSIS
		This script contains functionality used to test Notification Hubs ARM Templates output variables.
	...
	.EXAMPLE
		Default:
		C:\PS>.\NotificationHubs.output.tests.ps1
...
#region - Outputs
if (-not [string]::IsNullOrWhiteSpace($PSBoundParameters['notificationHubsNames']))
{
...
{
	Write-Output "Notification Hubs Resource Group: $($notificationHubsResourceGroup)"
}
else
{
	Write-Output "Notification Hubs Resource Group: []"
}
#endregion