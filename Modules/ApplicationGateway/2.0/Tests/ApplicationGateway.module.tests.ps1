<#
	.NOTES
		...
		File:		ApplicationGateway.module.tests.ps1

		Purpose:	Pester - Test Application Gateway ARM Templates
...
ForEach ($file in (Get-ChildItem (Join-Path -Path "$here" -ChildPath "*.parameter*.json") | Select-Object -ExpandProperty Name)) {
...
Describe "Template: $template - Application Gateway" -Tags -Unit {
	...
	Context "Parameter File Syntax" {
		...
		It "Has environment parameters file" {
			(Join-Path -Path "$here" -ChildPath "*.parameter*.json") | Should -Exist
		}
	...
	Context "Template and Parameter Compactibility" {
		...
		BeforeEach {
			...
			ForEach ($file in (Get-ChildItem (Join-Path -Path "$here" -ChildPath "*.parameter*.json") `
				| Select-Object -ExpandProperty Name)) {
